  ...
  for (i in 1:length(lam1s)) {
    ...
    if (lam1s[i] != 0) {
      ...
      for (j in 1:length(lam2s)) {
        if (p2 == 0) {
          beta <- diag(rep(g.out$wistuff$firstdiag, ncol(x))) %*% rob_cov(x, y, alternateCov = alternateCov) +
            g.out$wistuff$v %*% (diag(g.out$wistuff$diagsandwich) %*% ((t(g.out$wistuff$v)) %*% rob_cov(x, y, alternateCov = alternateCov)))
        } else if (p2 != 0 && p2 == 1) {
          if (j == 1) {
            l_one_res <- lasso_one(
