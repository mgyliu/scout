...
#' lambda1_max.
#' @param alternateCov character string describing which covariance estimator
#' to use. "default" corresponds to default covariance, "cellwise" corresponds
#' to cellwise covariance. These are the only two available options right now
...
get_lambda1_path <- function(X, p1, alternateCov = "default",
                             nlambda = 100, lambda_min_ratio = 0.001,
                             desc_frac = 0.8, max_iter = 100) {
  if (is.null(p1)) {
    return(0)
  } else if (p1 == 1) {
    lambda_max <- get_lambda1_max_glasso(X, alternateCov, desc_frac, max_iter)
...
#' @param max_iter maximum number of iterations to use to find the lowest
#' lambda_max in bisection search
get_lambda2_path <- function(X, Y, p2, alternateCov = "default",
                             nlambda = 100, lambda_min_ratio = 0.001,
                             desc_frac = 0.8, max_iter = 100) {
  if (is.null(p2)) {
    return(0)
  } else if (p2 == 1) {
    lambda_max <- get_lambda2_max_lasso(X, Y, alternateCov)
    return(get_lambda_path(lambda_max, nlambda, lambda_min_ratio))
  } else {
...
get_lambda2_max_lasso <- function(X, Y, alternateCov = "default", desc_frac = 0.8, max_iter = 100) {
  if (alternateCov == "default") {
    XTY <- t(X) %*% Y
  } else {
    Xw <- cellWise::wrap(X)$Xw
  ...
  lambda2_max_tmp <- max(abs(XTY) * 2)
  lambda2_max <- lambda2_max_tmp
  iter <- 0
  cond <- TRUE
  while (cond & iter < max_iter) {
    l_one_res <- lasso_one(
      rob_cov(X, alternateCov = alternateCov),
      rob_cov(X, Y, alternateCov = alternateCov),
      rho = lambda2_max_tmp
    )
